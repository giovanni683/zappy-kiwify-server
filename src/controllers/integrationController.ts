...
export async function createNotificationRule(req: Request, res: Response) {
  const { integrationId, accountId, event, message, adjustments } = req.body;
  if (!integrationId || !accountId || typeof event !== 'number' || !message || String(message).trim() === '') {
    return res.status(400).json({ error: 'Campos obrigatórios ausentes ou inválidos: integrationId, accountId, event, message.' });
  }
  try {
    const id = uuidv7();
    ...
    res.status(201).json({ success: true, id: rule.id, rule });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
}
